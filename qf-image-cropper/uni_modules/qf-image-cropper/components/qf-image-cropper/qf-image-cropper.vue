<template>
	<view class="image-cropper" @wheel="cropper.mousewheel">
		<canvas type="2d" id="imgCanvas" canvas-id="imgCanvas" class="img-canvas" :style="{
			width: `${imgWidth}px`,
			height: `${imgHeight}px`
		}"></canvas>
		<view class="pic-preview" :change:init="cropper.initObserver" :init="initData" @touchstart="cropper.touchstart" @touchmove="cropper.touchmove" @touchend="cropper.touchend">
			<image v-if="imgSrc" class="crop-image" :style="cropper.imageStyles" :src="imgSrc" webp></image>
			<view v-for="(item, index) in maskList" :key="item.id" class="crop-mask-block" :style="cropper.maskStylesList[index]"></view>
			<view v-if="showBorder" class="crop-border" :style="cropper.borderStyles"></view>
			<view v-if="radius > 0" class="crop-circle-box" :style="cropper.circleBoxStyles">
				<view class="crop-circle" :style="cropper.circleStyles"></view>
			</view>
			<block v-if="showGrid">
				<view v-for="(item, index) in gridList" :key="item.id" class="crop-grid" :style="cropper.gridStylesList[index]"></view>
			</block>
			<block v-if="showAngle">
				<view v-for="(item, index) in angleList" :key="item.id" class="crop-angle" :style="cropper.angleStylesList[index]">
					<view :style="[{
						width: `${angleSize}px`,
						height: `${angleSize}px`
					}]"></view>
	...
	export default {
		...
		methods: {
			...
			drawImage(ctx, image, callback) {
				// 生成图片的实际尺寸与截取区域比
				const scale = this.imgWidth / this.area.width;
			...
			draw2DImage(canvas, ctx, src, callback) {
				if(canvas) {
					...
					image.onload = () => {
						...
						callback && setTimeout(() => {
							callback();
						}, 1000); // 如果觉得`生成时间过长`或`出现生成图片空白`可尝试调整延迟时间
					};
					image.src = src;
				} else {
